      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_400px]">
            ...
            <tbody>
              {filtered.map((c) => (
                <tr
                  ...
                  className={`border-t border-border cursor-pointer ${selected === c.id ? "bg-accent/5" : "hover:bg-muted/30"}`}
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex rounded-full text-xs px-2 py-0.5 font-medium ${c.status === "active" ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}
                    >
                      {c.status}
                    </span>
                  </td>
        ...
        <aside className="rounded-lg border border-border bg-card p-5 h-fit">
          ...
          <div className="mt-6 grid grid-cols-2 gap-2">
            <button className="h-9 rounded-md border border-border text-sm hover:bg-secondary">
              Manage numbers
            </button>
            <button className="h-9 rounded-md border border-border text-sm hover:bg-secondary">
              Edit billing
            ...
            <button className="h-9 rounded-md border border-destructive/30 text-destructive text-sm hover:bg-destructive/5">
              Suspend
            </button>
          </div>
        </aside>
