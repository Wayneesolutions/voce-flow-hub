...
function ScriptReview() {
  ...
  return (
    <DashboardShell
      sidebar={null}
      title="Script review"
      subtitle={`${pendingScripts.length} scripts awaiting approval`}
    >
        ...
        <aside className="rounded-lg border border-border bg-card divide-y divide-border">
          {pendingScripts.map((s) => (
            <button
              ...
              className={`w-full text-left p-4 ${selected === s.id ? "bg-accent/5 border-l-2 border-l-accent" : "hover:bg-muted/30"}`}
            >
        ...
        <main className="rounded-lg border border-border bg-card">
          ...
          <div className="p-5 grid gap-5">
            <Section title="Company information">
              <p>
                Acme Plumbing is a residential & light-commercial plumbing service operating across
                the Greater Toronto Area. 12 years in business, 4.8★ on Google. Specializing in
                emergency calls and water heater installs.
              </p>
            </Section>
            ...
            <Section title="Goal">
              <p>
                Book a free in-home estimate within the next 7 days. Confirm address and preferred
                time window.
              </p>
            </Section>
            <Section title="Objection handling">
              <ul className="space-y-2">
                <Objection
                  q="We already have a plumber."
                  a="Totally understand — we're not asking you to switch. We offer a free second-opinion visit; many homeowners save 20–30% on quoted repairs."
                />
                <Objection
                  q="Send me an email instead."
                  a="Happy to — what's the best email? I'll also book a quick 10-min slot so the technician can call if anything's unclear."
                />
                <Objection
                  q="Is this an AI?"
                  a="Yes, I'm Acme's AI assistant. Want me to connect you to a human technician or finish booking your estimate?"
                />
              </ul>
            </Section>
            ...
            <Section title="Voice selection">
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div
                    key={v.n}
                    className={`rounded-md border p-3 ${v.on ? "border-accent bg-accent/5" : "border-border"}`}
                  >
          ...
          <div className="border-t border-border p-5 bg-muted/20">
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              <button className="h-10 px-4 inline-flex items-center gap-1.5 rounded-md border border-destructive/30 text-destructive text-sm font-medium hover:bg-destructive/5">
                <X className="h-4 w-4" />
                Reject
              </button>
              <button className="h-10 px-4 inline-flex items-center gap-1.5 rounded-md bg-success text-success-foreground text-sm font-medium hover:opacity-90">
