...
export const Route = createFileRoute("/login")({
  beforeLoad: () => {
    // Skip during SSR — localStorage is client-only
...
function LoginPage() {
  ...
  const handleForgotSubmit = async (e: FormEvent) => {
    ...
    setFpLoading(true);
    try {
      await authApi.tenantForgotPassword(fpEmail);
      setFpSent(true);
    } catch (err) {
      if (axios.isAxiosError(err) && err.response?.data?.error)
            ...
            <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
              ...
              {forgotMode ? (
                <div className="p-6 space-y-5">
                  <button
                    type="button"
                    onClick={() => { setForgotMode(false); setFpSent(false); setFpEmail(""); setFpError(""); }}
                  ...
                  {fpSent ? (
                    ...
                    <form onSubmit={handleForgotSubmit} className="space-y-4">
                      <div>
                        <p className="font-semibold text-sm mb-1">Forgot your password?</p>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        type="button"
                        onClick={() => { setForgotMode(true); setFpEmail(email); }}
