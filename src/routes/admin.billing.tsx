  ...
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          label="Total minutes"
          value={billingTotals.minutes.toLocaleString()}
          icon={<Timer className="h-4 w-4" />}
          delta={9}
        />
        <StatCard
          label="Revenue"
          value={`$${billingTotals.revenue.toLocaleString()}`}
          icon={<DollarSign className="h-4 w-4" />}
          delta={14}
        />
        <StatCard
          label="Platform cost"
          value={`$${billingTotals.cost.toLocaleString()}`}
        ...
        <StatCard
          label="Gross profit"
          value={`$${billingTotals.profit.toLocaleString()}`}
        ...
        <div className="p-5 border-b border-border flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Cost calculated at $0.12/min platform cost
            </div>
          </div>
        </div>
          ...
          <tbody>
            ...
            <tr className="border-t-2 border-border bg-primary text-primary-foreground">
              ...
              <td className="px-3 py-3 text-right tabular-nums font-semibold">
                {billingTotals.minutes.toLocaleString()}
              </td>
              <td className="px-3 py-3 text-right tabular-nums">—</td>
              <td className="px-3 py-3 text-right tabular-nums font-semibold">
                ${billingTotals.revenue.toLocaleString()}
              </td>
              <td className="px-3 py-3 text-right tabular-nums">
                ${billingTotals.cost.toLocaleString()}
              </td>
              <td className="px-3 py-3 text-right tabular-nums font-semibold">
                ${billingTotals.profit.toLocaleString()}
              </td>
